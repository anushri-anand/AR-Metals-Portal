...
export default function PurchaseOrderEntryForm({
  ...
  const poAmountAed = useMemo(() => {
    return poAmount * Number(form.exchangeRate || 0)
  }, [poAmount, form.exchangeRate])

  const poAmountIncVatAed = useMemo(() => {
    return poAmountAed + Number(form.vatAed || 0)
  ...
  function buildItemDescription() {
    return items
      .map((item, index) => {
        ...
        const depreciationEndDate = getDepreciationEndDate(
          item.depreciationStartDate,
          item.depreciationPeriodYears
        )

        const baseDescription = `${index + 1}. ${item.item} | Qty: ${item.quantity} | Unit: ${item.unit} | Rate: ${item.rate} | Amount: ${amount}`

        if (!includeDepreciation) {
          return baseDescription
        }

        return `${baseDescription} | Depreciation Period: ${item.depreciationPeriodYears} years | Depreciation Start: ${item.depreciationStartDate} | Depreciation End: ${depreciationEndDate}`
      })
      .join('\n')
  }
    ...
    try {
      await fetchAPI('/procurement/purchase-order/entry/', {
        ...
        body: JSON.stringify({
          ...
          project_number: includeProjectFields ? form.projectNumber : 'Inventory',
          project_name: includeProjectFields ? form.projectName : 'Inventory',
          ...
          po_amount: formatMoney(poAmount),
              ...
              return (
                <div key={index} className="rounded-xl border border-slate-200 p-6">
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    {includeDepreciation && (
                      <>
                        <Field label="Depreciation Period (Year)">
                          <input
                            type="number"
                            min="1"
                        ...
                        <Field label="Depreciation End Date">
                          <input
...
function getDepreciationEndDate(startDate: string, years: string) {
  const yearCount = Number(years || 0)

  if (!startDate || !yearCount) {
    return ''
  }

  const endDate = new Date(`${startDate}T00:00:00`)
  endDate.setFullYear(endDate.getFullYear() + yearCount)
