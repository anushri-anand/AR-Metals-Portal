...
export default function ActualIncurredCostClient() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const pettyCashVouchers = await fetchAPI('/procurement/petty-cash/')

        const actualIncurredRows = (Array.isArray(pettyCashVouchers) ? pettyCashVouchers : [])
          .flatMap((voucher) => buildActualIncurredRows(voucher as PettyCashVoucher))
          .sort((left, right) => {
          ...
          <table className="min-w-[1560px] border-collapse text-sm">
            <thead className="sticky top-0 z-10 bg-slate-100 text-left text-slate-700">
              <tr>
                ...
                <HeaderCell>Petty Cash Voucher #</HeaderCell>
              ...
              {loading ? (
                ...
                filteredRows.map((row) => (
                  <tr key={row.key} className="border-b border-slate-200">
                    ...
                    <BodyCell>{row.voucherNumber || '-'}</BodyCell>
...
function buildActualIncurredRows(voucher: PettyCashVoucher) {
  return (voucher.items || [])
    .map((item) => {
      const quantity = toNumber(item.quantity)
      const rate = toNumber(item.rate)
      const amount = toNumber(item.amount_exc_vat) || quantity * rate
